class Library:
    ...
    def borrowBook(self, name, bookname):
        if bookname not in self.books:
            print(
                f"{bookname} BOOK IS NOT AVAILABLE OR TAKEN BY SOMEONE ELSE, PLEASE WAIT THE BOOK IS AVAILABLE.\n")
        else:
...
if __name__ == "__main__":
    ...
    GreySenselibrary = Library(
        ["Vistas", "Invention", "Rich&poor", "Indian", "Macroeconomics",     "Microeconomics", "In Search of Lost Time", "Ulysses", "Don Quixote", "One Hundred Years of Solitude", "The Great Gatsby", "Moby Dick", "War and Peace", "Hamlet", "Wings of fire"])
    student = Student()
    track = []

    print("\t\t\t\t\t\t\t♦♦♦♦♦♦♦ WELCOME TO THE GreySense LIBRARY ♦♦♦♦♦♦♦\n")
    ...
    while (True):
        ...
        try:
            ...
            if usr_response == 1:           # listing
                GreySenselibrary.displayAvailableBooks()
            elif usr_response == 2:         # borrow
                GreySenselibrary.borrowBook(
                    input("Enter your name: "), student.requestBook())
            elif usr_response == 3:         # return
                GreySenselibrary.returnBook(student.returnBook())
            elif usr_response == 4:         # donate
                GreySenselibrary.donateBook(student.donateBook())
